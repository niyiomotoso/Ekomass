...
$insert_image="UPDATE  inventory SET item='$proname', price='$price', qtyleft='$qty',description='$description',category='$category', application='".$application."', datasheet='".$datasheet."', value='".$value."', kwd='$kwd', uploaded= 0 WHERE ind ='$ind' ";
if($dbconn-> query($insert_image))
header("location: ../admin_show_all_items");
}
else
{
...
while(file_exists('../inventory/'.$filepath))
{
    $fileName = uniqid() . '.' . $fileData['extension'];
    $filepath = ('images/'.$fileName);
}
...
if(move_uploaded_file($_FILES["myimage"]["tmp_name"], '../inventory/'.$filepath))
{
// If file has uploaded successfully, store its name in data base

	unlink('../inventory/'.$formerpic);
	smart_resize_image('../inventory/'.$filepath,
                              $string             = null,
                              $width              = 500, 
                              $height             = 500, 
                              $proportional       = false, 
